...
enum HelpContent {
    ...
    static let sections: [HelpSection] = [
        ...
        HelpSection(
            ...
            ・整形の文字数（⌃E で使う字数。初期値20。ウインドウ下部からも選べます）
        ...
        HelpSection(
            ...
            ・ウインドウ下部の右側では、整形の文字数（5〜40字）・自動保存の間隔（オフ／1／3／5／10／30分）・文字コード・改行コードをその場で選び直せます。いちばん右の「編集」「閲覧」を押すと閲覧モードを切り替えられます。整形の文字数は設定パネルからも指定でき、どちらで変えても同じ値です（設定パネルの入力欄は5〜40の外の値も受け付けます。その場合はその値も一覧に出ます）。
